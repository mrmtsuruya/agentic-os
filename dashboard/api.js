const api = {
  ...
  // P5: Studio / YouTube autopilot
  studioResearch: (query) => api.get(`/api/studio/research?query=${encodeURIComponent(query)}`),
  studioSeo: (topic) => api.post('/api/studio/seo', { topic }),
  studioScript: (topic) => api.post('/api/studio/script', { topic }),
  studioGenerate: (topic) => api.post('/api/studio/generate', { topic }),
  ...
  intelSignals: () => api.get('/api/intel/signals'),
  intelAddSignal: (s) => api.post('/api/intel/signals', s),
  intelRefresh: () => api.get('/api/intel/refresh'),
  intelResearch: (q, mid) => api.post('/api/intel/research', { query: q, related_mission: mid || '' }),
  intelGraphNodes: () => api.get('/api/intel/graph/nodes'),
  intelGraphLookup: (q) => api.get(`/api/intel/graph/lookup?q=${encodeURIComponent(q)}`),
  intelGraphBuild: () => api.get('/api/intel/graph/build'),
  intelPlugins: () => api.get('/api/intel/plugins'),
  intelRegisterPlugin: (p) => api.post('/api/intel/plugins', p),
  intelEnablePlugin: (pid) => api.post(`/api/intel/plugins/${pid}/enable`, {}),
  intelDisablePlugin: (pid) => api.post(`/api/intel/plugins/${pid}/disable`, {}),
  intelPluginAudit: () => api.get('/api/intel/plugins/audit'),
